...
//O(N)
int diameterON(node* root,int maxi){
    if(root == NULL)
        return 0;
    
    //root
     
    int op2 = diameterON(root->left,maxi);//left
    int op3 = diameterON(root->right,maxi);//right
    maxi = max(maxi,op2+op3);
    return 1+ max(op3,op2);
}
